...
class CollectorRuntime:
    ...
    def start(self):
        ...
        try:
            ...
            self.logs = StagingLogs(self.roots['log_root']/node)
    ...
    def step(self):
        ...
        self.health = HealthSnapshot(self.config.collectors[0].collector_id, now,
            ...
            error_state=self.error_state, disk_path_accessible=True, disk_free_bytes=disk.free,
...
def main():
    ...
    try:
        if args.dry_run:
            print(json.dumps(dry_run(args.config, args.runtime)))
            return 0
