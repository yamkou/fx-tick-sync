...
def cleanup_expired_zip(folder_id):
    """7日以上前に作成されたZIPファイルを検索し削除（リンク無効化）"""
    ...
    for f in res.get('files', []):
        print(f"  -> 有効期限(7日)切れのため削除: {f['name']}")
...
def convert_to_mt_csv(raw_csv_path, mt4_path, mt5_path):
    """DukascopyデータをMT4/MT5用のCSV形式に整形"""
    df = pd.read_csv(raw_csv_path)
    # カラム名マッピング（Dukascopyの仕様に合わせて調整）
    # 通常: time / timestamp, bid, ask
    time_col = [c for c in df.columns if 'time' in c.lower()][0]
    df['dt'] = pd.to_datetime(df[time_col])

    if mt4_path:
        # MT4形式: YYYY.MM.DD HH:MM:SS.fff,Bid,Ask,0,0
        df_mt4 = pd.DataFrame({
    ...
    if mt5_path:
        # MT5形式: タブ区切りまたはカンマ区切り (Date, Time, Bid, Ask, Volume)
        df_mt5 = pd.DataFrame({
            ...
            try:
                ...
                if df is not None and not df.empty and len(df) > 0:
                    ...
                    if TARGET_FORMAT in ['MT4', 'BOTH']:
                        ...
                        convert_to_mt_csv(new_csv, mt4_path=mt4_csv, mt5_path=None)
                        files_to_zip.append((mt4_csv, f"{folder_name}_MT4.csv"))

                    if TARGET_FORMAT in ['MT5', 'BOTH']:
                        mt5_csv = os.path.join(tmpdir, f"{folder_name}_MT5.txt")
                        convert_to_mt_csv(new_csv, mt4_path=None, mt5_path=mt5_csv)
                        files_to_zip.append((mt5_csv, f"{folder_name}_MT5.txt"))

            except Exception as e:
                print(f"[{folder_name}] エラー: {e}")

        # メール送信用 暗号化ZIP生成
        if files_to_zip and RECIPIENT_EMAIL:
            print(f"\n暗号化ZIPを作成中...")
            zip_filename = f"TickData_{TARGET_FORMAT}_{now.strftime('%Y%m%d_%H%M')}.zip"
            zip_path = os.path.join(tmpdir, zip_filename)

            # パスワード決定（指定がなければ英数10文字を自動生成）
